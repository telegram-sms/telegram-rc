...
public class send_spam_service extends Service {
    Context context;
    String request_uri;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Notification notification = public_func.get_notification_obj(context, "Send filtered spam ");
        ...
        new Thread(() -> {
            ...
            while (resend_list.size() != 0) {
                if (public_func.check_network_status(context)) {
                    resend_list = Paper.book().read("spam_sms_list", new ArrayList<>());
    ...
    class stop_notify_receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (Objects.equals(intent.getAction(), public_func.BROADCAST_STOP_SERVICE)) {
                Log.i("send_spam_loop", "Received stop signal, quitting now...");
                stopSelf();
            }
        }
    }
