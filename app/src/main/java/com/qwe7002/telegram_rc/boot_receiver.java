...
public class boot_receiver extends BroadcastReceiver {
    ...
    public void onReceive(@NotNull final Context context, @NotNull Intent intent) {
        ...
        if (sharedPreferences.getBoolean("initialized", false)) {
            log.write_log(context, "Received [" + intent.getAction() + "] broadcast, starting background service.");
            ...
            if (sharedPreferences.getBoolean("root", false)) {
                ...
                if (Boolean.TRUE.equals(Paper.book("temp").read("wifi_open", false))) {
                    ...
                    remote_control.enable_vpn_ap(wifiManager);
                }
                if (Boolean.TRUE.equals(Paper.book("temp").read("tether_open", false))) {
                    remote_control.enable_tether(context, Paper.book("temp").read("tether_mode", TetherManager.TetherMode.TETHERING_WIFI));
                }
            }
        }
